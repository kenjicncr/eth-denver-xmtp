...
export const ConversationList = ({
  ...
  !messages?.length && isLoading ? (
    <div className="w-full overflow-hidden h-full flex flex-col justify-start sm:w-full bg-background">
      {Array.from({ length: 12 }).map((_, idx) => (
        <MessagePreviewCard key={idx} isLoading activeTab={activeTab} />
      ))}
    </div>
  ) : !messages.length && !isLoading && !hasRecipientEnteredValue ? (
    <div className="w-full overflow-hidden sm:w-full sm:p-4 md:p-8 border border-zinc-900 h-full">
      {activeTab === "messages" ? (
    ...
    <Virtuoso
      className="sm:w-full flex flex-col h-full bg-background border-x"
